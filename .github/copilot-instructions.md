...
`process_audio.py` assumes `ffmpeg` is on `PATH`, CUDA-backed `faster-whisper` is available, and the `audio_separator` model assets can be cached under `.\temp`. `translate_subs.py` assumes a local Ollama server at `http://localhost:11434` with the configured model available; the current default is `qwen3:14b`.
...
## Key conventions
- Cache files control reruns. If `<base>.json` already exists, `process_audio.py` skips separation/transcription; if `<base>_translated.json` exists, `translate_subs.py` skips Ollama calls. Delete those files to force regeneration.
